...
if __name__ == '__main__':
    ...
    parser.add_argument("mode", help="which mode to run in: \"opt\" to train the network," +
                                     ...
                                     " \"compile_rank\" to compile all saved rank history from previous ranks/evals,"
                                     " or \"hist\" to view the history of training.",
                        choices=["opt", "self", "play", "eval", "rank", "hist", "compile_rank"])
    ...
    if args.mode == "opt":
        ...
        history.start()
    elif args.mode == "compile_rank":
        from worker import compile_rank
        compile_rank.start()